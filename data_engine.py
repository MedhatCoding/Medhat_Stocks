...
class DataEngine:
    ...
    def get_egx_symbols(self):
        ...
        try:
            response = requests.get(url, params={"api_token": self.eodhd_api_key, "fmt": "json"}, timeout=30)
    ...
    def get_stock_history(self, symbol, days=365):
        ...
        try:
            response = requests.get(url, params={"api_token": self.eodhd_api_key, "fmt": "json", "period": "d", "order": "d"}, timeout=30)
            response.raise_for_status()
            data = response.json()
            ...
            return {"success": False, "symbol": symbol, "error": str(exc), "data": []}

    def get_latest_price(self, symbol):
        result = self.get_stock_history(symbol, days=1)
        if not result["success"] or not result["data"]:
            return {"success": False, "symbol": symbol, "price": None, "error": result.get("error", "لا توجد بيانات")}
        row = result["data"][0]
        return {
            "success": True,
