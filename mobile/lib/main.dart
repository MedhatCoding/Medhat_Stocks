...
class _AppState extends State<App>{bool dark=true;@override void initState(){super.initState();SharedPreferences.getInstance().then((p){if(mounted)setState(()=>dark=p.getBool('dark')??true);});}Future<void>theme(bool v)async{final p=await SharedPreferences.getInstance();await p.setBool('dark',v);if(mounted)setState(()=>dark=v);}@override Widget build(BuildContext c)=>MaterialApp(debugShowCheckedModeBanner:false,title:'مدحت ستوكس',locale:const Locale('ar'),supportedLocales:const[Locale('ar')],themeMode:dark?ThemeMode.dark:ThemeMode.light,theme:ThemeData(useMaterial3:true,colorScheme:ColorScheme.fromSeed(seedColor:const Color(0xFFF5C542))),darkTheme:ThemeData(useMaterial3:true,brightness:Brightness.dark,colorScheme:ColorScheme.fromSeed(seedColor:const Color(0xFFF5C542),brightness:Brightness.dark)),home:Shell(onTheme:theme));}
...
class _HomeState extends State<Home>{
 Map<String,dynamic>?m;List<dynamic>o=[];bool loading=true;
 @override void initState(){super.initState();load();}
 Future<void>load()async{setState(()=>loading=true);try{final r=await Future.wait([Api.get('/market/context'),Api.get('/opportunities?limit=5')]);if(mounted)setState((){m=r[0];o=r[1]['data']??[];loading=false;});}catch(_){if(mounted)setState(()=>loading=false);}}
 @override Widget build(BuildContext c)=>RefreshIndicator(onRefresh:load,child:ListView(padding:const EdgeInsets.fromLTRB(18,12,18,24),children:[
  Row(children:[Builder(builder:(x)=>IconButton(onPressed:()=>Scaffold.of(x).openDrawer(),icon:const Icon(Icons.menu_rounded))),const Expanded(child:Text('مدحت ستوكس',style:TextStyle(fontSize:26,fontWeight:FontWeight.w900))),IconButton(onPressed:load,icon:const Icon(Icons.refresh_rounded))]),
  const SizedBox(height:8),
  Container(padding:const EdgeInsets.all(20),decoration:BoxDecoration(borderRadius:BorderRadius.circular(24),gradient:const LinearGradient(colors:[Color(0xFF182F49),Color(0xFF081321)])),
   child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
    Row(children:[Container(padding:const EdgeInsets.all(10),decoration:BoxDecoration(color:Color(0x26F5C542),shape:BoxShape.circle),child:const Icon(Icons.candlestick_chart,color:Color(0xFFF5C542))),const SizedBox(width:12),const Text('تحليل ما قبل الافتتاح',style:TextStyle(color:Colors.white,fontSize:18,fontWeight:FontWeight.w800))]),
    const SizedBox(height:18),const Text('حالة السوق',style:TextStyle(color:Colors.white60)),const SizedBox(height:4),
    Text(m?['regime']?.toString()??(loading?'جاري التحميل…':'—'),style:const TextStyle(color:Colors.white,fontSize:29,fontWeight:FontWeight.w900)),
    const SizedBox(height:12),Row(children:[_metric('العائد 20 جلسة',fmt(m?['return20'])+'%'),_metric('آخر جلسة',m?['date']?.toString()??'—':'—')])
   ])),
  const SizedBox(height:22),Row(children:[const Expanded(child:Text('الفرص المرشحة',style:TextStyle(fontSize:22,fontWeight:FontWeight.w900))),TextButton(onPressed:()=>go(1),child:const Text('عرض الكل'))]),
  if(o.isEmpty&&!loading)const Box('لا توجد فرص مؤهلة حالياً.'),
  ...o.map((x)=>Opportunity(Map<String,dynamic>.from(x))),
  const SizedBox(height:8),Row(children:[Expanded(child:Act('تحليل سهم',Icons.search,()=>go(2))),const SizedBox(width:10),Expanded(child:Act('المتابعة',Icons.star,()=>go(3)))])
 ]));
 Widget _metric(String a,String b)=>Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(a,style:const TextStyle(color:Colors.white54,fontSize:11)),const SizedBox(height:3),Text(b,style:const TextStyle(color:Colors.white,fontWeight:FontWeight.w800))]));
}
...
class Opportunity extends StatelessWidget{final Map<String,dynamic>x;const Opportunity(this.x,{super.key});@override Widget build(BuildContext c){final score=numVal(x['opportunity_score'])??0;return Card(shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(18)),child:InkWell(borderRadius:BorderRadius.circular(18),onTap:()=>Navigator.push(c,MaterialPageRoute(builder:(_)=>Details(symbol:x['symbol'].toString(),name:x['name']?.toString()??''))),child:Padding(padding:const EdgeInsets.all(15),child:Row(children:[Container(width:48,height:48,decoration:BoxDecoration(color:const Color(0x26F5C542),borderRadius:BorderRadius.circular(14)),child:Center(child:Text(x['symbol'].toString().substring(0,1),style:const TextStyle(color:Color(0xFFF5C542),fontSize:20,fontWeight:FontWeight.w900)))),const SizedBox(width:13),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(x['symbol'].toString(),style:const TextStyle(fontSize:17,fontWeight:FontWeight.w900)),const SizedBox(height:4),Text('RSI '+fmt(x['rsi14'])+' • ارتداد '+fmt(x['rebound_score'])+' • مخاطر '+fmt(x['risk_score']),style:TextStyle(fontSize:11,color:Theme.of(c).colorScheme.onSurfaceVariant))])),Column(crossAxisAlignment:CrossAxisAlignment.end,children:[Text(fmt(score),style:TextStyle(fontSize:22,fontWeight:FontWeight.w900,color:score>=70?const Color(0xFF25C58A):const Color(0xFFF5C542))),const Text('درجة الفرصة',style:TextStyle(fontSize:9))])]))));}}
